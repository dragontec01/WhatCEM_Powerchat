...
app.use((req, res, next) => {
  ...
  console.log({headersReceived: req.headers, queryParamsReceived: req.query});
    ...
    setTimeout(async () => {
      try {
        ...
        if (process.env.NODE_ENV !== 'production') {
          ...
          logger.info('migration', 'Skipping migrations - handled by docker-entrypoint.sh in production');
        }
        ...
        try {
          ...
          logger.info('backup', '✅ Backup scheduler initialized successfully');
        } catch (error) {
          logger.error('backup', '❌ Backup scheduler initialization failed:', error);
        }

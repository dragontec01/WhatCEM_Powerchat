...
async function getEnhancedGroupMetadata(sock: WASocket, groupJid: string) {
  try {
    ...
    for (const participant of metadata.participants) {
      ...
      logger.info('whatsapp-service', `Enhancing participant data for ${participant.id}`);
...
async function processMessageThroughFlowExecutor(
  ...
  try {
    ...
    logger.debug('whatsapp-service',`[msg:${message.id || 'unknown'}] Processing through flow executor:`, {
    ...
    logger.error('whatsapp-service', `[msg:${message.id || 'unknown'}] Error in processMessageThroughFlowExecutor:`, error);
    throw error;
  }
}
  ...
  try {
    ...
    contact = await storage.getContactByIdentifier(cleanPhoneNumber, 'whatsapp');
      ...
      if (!contact) {
        ...
        logger.info('whatsapp-service', `[conn:${connectionId}] [msg:${messageId}] Contact created:`, {
          contactId: contact.id,
          phone: contact.phone,
          name: contact.name,
      ...
      if (!conversation) {
        ...
        logger.info('whatsapp-service', `[conn:${connectionId}] [msg:${messageId}] Conversation found:`, {
          conversationId: conversation.id,
          contactId: contact.id,
          channelId: connectionId,
          isGroup: false
    ...
    if (!isGroupChat && !isFromMe) {
      try {
        ...
        if (channelConnection && contact) {
          logger.info('whatsapp-service',`[conn:${connectionId}] [msg:${message.id}] Before scheduling processing:`, {
            remoteJid: resolvedRemoteJid, // Use resolved JID (actual phone number)
            canonicalIdentifier: cleanPhoneNumber,
...
export async function sendWhatsAppMessage(
  ...
  try {
    ...
    logger.info('whatsapp-service', `[conn:${connectionId}] Final reply target assignment:`, {
      originalTo: to,
